...
const getPhoto = (num) => `/images/photos/course-au-large/${num}.webp`;

const gallery = [
  1, 2, 3, 4, 7, 14, 11, 22, 5, 6, 21, 23, 9, 18, 16, 24, 25, 29, 12, 13,
  8, 10, 26, 27, 28, 15, 20,
];
...
export default function CourseAuLargePage() {
  ...
  const GalleryImage = ({ num, ratio = "aspect-[3/2]", className = "" }) => (
    <motion.button
      type="button"
      onClick={() => openImage(num)}
      className={`group relative block w-full cursor-zoom-in overflow-hidden rounded-[2rem] bg-stone-200 text-left shadow-xl shadow-stone-900/5 ${ratio} ${className}`}
    ...
    >
      <Image
        src={getPhoto(num)}
        alt={`Course au large - photo ${num} par Alexis Yvonnou`}
        fill
        sizes="(max-width: 768px) 100vw, 50vw"
        className="object-cover transition duration-700 ease-out group-hover:scale-[1.025] group-hover:brightness-90"
      />

      <div className="pointer-events-none absolute inset-0 opacity-0 transition duration-500 group-hover:opacity-100">
        <div className="absolute inset-0 bg-black/10" />
        <div className="absolute bottom-4 right-4 rounded-full bg-white/90 px-4 py-2 text-xs font-semibold text-stone-950 backdrop-blur-md">
          Voir
        </div>
      </div>
    </motion.button>
  ...
  return (
    <main className="min-h-screen bg-[#f7f4ef] text-stone-950">
      ...
      <section className="relative mt-[92px] h-[calc(88vh-92px)] min-h-[560px] w-full overflow-hidden">
        <button
          type="button"
          onClick={() => openImage(1)}
        ...
        >
          <Image
            src={getPhoto(1)}
            alt="Course au large - photographie Alexis Yvonnou"
            fill
            priority
            sizes="100vw"
        ...
        <div className="pointer-events-none absolute bottom-8 left-5 right-5 text-white md:bottom-14 md:left-10 md:right-10">
          ...
          <p className="mt-7 max-w-3xl text-2xl font-medium leading-tight tracking-[-0.04em] text-white/86 md:text-4xl">
            Entre tension, préparation et vie des marins.
          </p>

          <p className="mt-6 max-w-2xl text-base leading-8 text-white/70 md:text-lg">
            Des pontons aux retours de mer, une galerie dédiée à la voile de
            compétition et aux récits qui se jouent autour des bateaux.
          </p>
        </div>
      </section>
      ...
      <section className="px-5 pb-24 md:px-8 md:pb-28">
        <div className="mx-auto max-w-7xl">
          <div className="relative min-h-[560px] overflow-hidden rounded-[3rem] bg-stone-950 text-white shadow-2xl shadow-stone-900/20 md:min-h-[660px]">
            <Image
              src={getPhoto(23)}
              alt="Photographier la course au large"
              fill
              sizes="100vw"
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8 md:p-14">
              ...
              <h2 className="max-w-5xl text-5xl font-semibold leading-[0.9] tracking-[-0.065em] md:text-7xl">
                Photographier la course au large, c’est raconter un univers.
              </h2>

              <p className="mt-8 max-w-2xl text-lg leading-9 text-white/70">
                Les départs, les retours, les préparations, les détails, les
                gestes, les visages et tout ce qui existe autour de la
                performance.
              </p>
            </div>
          </div>
